...
def get_result(correct_chunks, true_chunks, pred_chunks,
    correct_counts, true_counts, pred_counts,file_name, verbose=True):
    """
    ...
    #====
    print("processed %i tokens with %i phrases; " % (sum_true_counts, sum_true_chunks), end='',file=outfile)
    print("found: %i phrases; correct: %i.\n" % (sum_pred_chunks, sum_correct_chunks), end='',file=outfile)
    
    print("accuracy: %6.2f%%; (non-O)" % (100*nonO_correct_counts/nonO_true_counts),file=outfile)
    print("accuracy: %6.2f%%; " % (100*sum_correct_counts/sum_true_counts), end='',file=outfile)
    print("precision: %6.2f%%; recall: %6.2f%%; FB1: %6.2f" % (prec, rec, f1),file=outfile)
    ...
    for t in chunk_types:
        ...
        #----
        print("%17s: " %t , end='',file=outfile)
        print("precision: %6.2f%%; recall: %6.2f%%; FB1: %6.2f" %
                    (prec, rec, f1), end='',file=outfile)
        print("  %d" % pred_chunks[t],file=outfile)
...
def evaluate(true_seqs, pred_seqs, verbose=True,file_name='.'):
    ...
    result = get_result(correct_chunks, true_chunks, pred_chunks,
        correct_counts, true_counts, pred_counts, verbose=verbose,file_name=file_name)
    return result
